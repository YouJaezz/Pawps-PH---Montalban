...
export function PayrollPrintSlipLink(props: {
  ...
  const day = props.periodDay ?? 0;
  const dayQuery = day > 0 ? `&day=${day}` : "";
  const href = `/payroll/slip/${props.userId}?year=${props.year}&month=${props.month}&half=${props.half}${dayQuery}`;
