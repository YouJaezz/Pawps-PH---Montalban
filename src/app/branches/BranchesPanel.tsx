        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <form action={createBranch} className="mt-5 space-y-4">
            ...
            <label className="flex items-center gap-3 text-sm text-zinc-200">
              <input type="checkbox" name="isDefault" className="size-4 accent-white" />
      ...
      <div className="lg:col-span-3">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-sm font-medium text-zinc-100">Branch list</div>
              <div className="mt-1 text-xs text-zinc-400">
                Walk-in sales deduct from the default branch. Move stock between branches
                from Inventory.
              </div>
            </div>
                ...
                {tableRows.length === 0 ? (
                  ...
                  tableRows.map((b) => (
                    <tr key={b.id} className="align-top hover:bg-white/[0.02]">
                      ...
                      <td className="px-2 py-2 text-zinc-400">{b.location ?? "—"}</td>
