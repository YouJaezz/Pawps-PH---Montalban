  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-4">
      ...
      <p className="mt-0.5 text-[11px] text-zinc-500">
        Fee = max(base + km × rate, minimum) + extras
      </p>
      <form action={updateTransportPricing} className="mt-3 grid grid-cols-3 gap-2">
        ...
        <button
          type="submit"
          className="col-span-3 rounded border border-white/10 py-1.5 text-xs text-zinc-200 hover:bg-white/5"
        >
          Save rates — currently base {formatPhpFromCents(props.baseFeeCents)} +{" "}
          {formatPhpFromCents(props.perKmCents)}/km
        </button>
      </form>
    </div>
  );
